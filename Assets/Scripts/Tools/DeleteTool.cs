...
public class DeleteTool : MonoBehaviour
{
    ...
    private Color invalidColor = new Color(1f, 0.3f, 0f, 0.5f);
    ...
    {
        if (!isHeld) return;
        if (Keyboard.current != null && Keyboard.current.tKey.wasPressedThisFrame) TryDelete();
        UpdateGhostPosition();
    }
    ...
    void TryDelete()
    {
        ...
        {
            // Play destroy sound at the path block's world position before removing it
