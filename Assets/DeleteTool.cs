...
public class DeleteTool : MonoBehaviour
{
    ...
    private Color validColor   = new Color(0f,   1f,   0f,   0.5f); // green = hovering a path
    private Color invalidColor = new Color(1f,   0.3f, 0f,   0.5f); // orange = not a path
    ...
    void UpdateGhostPosition()
    {
        ...
        if (!TryGetPlacementPoint(out Vector3 placementPoint))
        {
            ...
            {
                Debug.LogWarning("DeleteTool has no placement point source. Assign Placement Ray Interactor or enable Allow Mouse Placement Fallback.");
